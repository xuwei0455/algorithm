// Number     : 164 
// Title      : Maximum Gap
// Source     : https://leetcode.com/problems/maximum-gap/description/
// Tags       : Sort
// Difficulty : Hard
// Author     : William
// Date       : 2017-12-08
...
int maximumGap(vector<int> &num) {
    ...
    // Divide the interval [min, max] int n "buckets" of equal size = (max - min) / n
    const int bucket_size = (max - min) / num.size() + 1;

    // For each of remaining n-2 numbers, determin in which bucket it falls .
    ...
    for (int i = 1; i < buckets.size(); i++)
    {
        ...
        int gap = buckets[i][0] - buckets[pre_idx][1];
